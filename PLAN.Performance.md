...
状态：2026-09-25 本地可完成部分全部完成（§68～§79；§79 审计清单 15 项均已处理）。§5 仍开放的 6 项均非本机可做：4 项需要硬件 GPU（kernel 时间戳、PERF-08/09 性能验收、GPU direct 与 CPU FFT 比较、真实硬件基准），1 项为可选的实数/GPU FFT 实验，1 项待负责人决定（mesh-render 写出重叠去留与 `gpu_min_pixels` 默认值）。另有 §79 末尾的负责人决定 (a)～(d)。前文“源码确认”描述初始基线，当前实现以各批记录和源码为准。
...
- [x] 在 release 构建下记录 load/parse、prepare、broad phase、narrow phase、voxelize、S2、PCA、transform、encode/write 各阶段 wall time。 （完成：`pipeline/timing.rs` 的 `[Timing]` 阶段行，§66）
- [ ] GPU 单列 adapter/device/pipeline 初始化、上传、kernel、回读和 CPU 后处理。时间戳查询仅在设备支持时使用；队列提交时间不能冒充 kernel 时间。 （主机侧完成：初始化、上传/回读字节与等待时间，§79 第 (4) 项；kernel 时间戳需硬件，保持开放）
- [x] 记录实际线程池 worker 数、岛数、适配器类型、输入规模、拒绝次数、精确查询数、上传/下载字节、峰值 RSS 与程序所分配的 GPU 资源估算值。后者不能称为驱动实际显存占用。 （完成：workers/peak_rss/岛数/适配器类别/拒绝与查询计数/传输字节/逻辑 GPU 估算，§66、§79）
- [x] 为纯内核提供稳定输入的基准入口；端到端基准使用独立输出目录，避免覆盖已有数据。 （完成：各 ignored release 基准与 `perf_matrix.py` 临时目录，§66、§72）
- [x] 区分首次运行和资源复用后的运行。MC/SA 使用可记录的基准种子和相同求值预算；这不要求立即改变公开 RNG 协议。 （完成：矩阵冷/暖运行；optimize/pack 种子，§66、§79）
...
- [x] 建立每次运行的 CPU 执行上下文，把计算阶段及 CPU 回退都纳入同一个 worker 预算；日志区分请求值与实际 worker 数。 （完成：`run_in_cpu_pool` 覆盖全部 CPU 管线，§79 第 (2) 项）
...
- [x] 按“岛间并行”与“岛内 S2 并行”组合测量，避免大量空闲池和竞争；GPU 调度还需要独立的在途任务/显存预算。（完成：§79 岛数扫描；GPU 在途预算为单设备互斥 + 逐次求值预算检查，§37）
- [x] 小列表/小网格使用串行阈值；阈值来自调度开销基准，而不是对所有循环套 `par_iter`。 （完成：各阈值均来自调度开销实测，§68、§72、§74）
...
- [x] 用 checked arithmetic 计算每种方法的输入、输出、中间值、staging 和同时在途任务的峰值字节。 （完成：mc/exact/render/crop 规划器，§36、§52、§78）
- [x] 分开检查 device 的单 buffer、单 binding、dispatch 三维上限，以及用户对整项任务的资源预算。 （完成：§36、§53）
- [x] 按预算确定批大小。用 `base_index`/局部坐标把一维大 dispatch 分成多批或二维网格；不能仅把溢出的尺寸强转成 `u32`。 （完成：§42、§52、§79 第 (5) 项、crop 分块 §78）
- [x] 接近 32 位计数上限的配对/采样分块计数，在 CPU 用 `u64` 合并，或使用经验证的多阶段整数归约。 （完成：工作组整数归约 + CPU u64 合并，§41、§45）
- [x] 计算 API 返回显式错误；读取 map 回调结果，在合适位置使用 wgpu error scope 并处理设备丢失。禁止以全局 `catch_unwind` 作为正常 GPU 调度策略。 （完成：§14、§15；设备丢失逐出与重建测试 §79 第 (6) 项）
- [x] 预算超限优先缩小批次，无法分块则按配置回退；显式禁用回退时返回错误。中途失败应丢弃不完整输出，不能混用旧缓冲内容。 （完成：MC 半径批缩小、crop 分块与 halo 重试、scene 条带，§78、§79）
...
- [x] 先为布局、129 个及更多半径、超过 200,000 offsets、位移超出维度、超过 64 个交点、大坐标小几何、非正 pitch 建立独立反例。 （完成：`tests/gpu_s2_boundaries_tests.rs`，§12、§13）
- [x] exact 使用小网格穷举 oracle；MC 使用相同采样点或相同统计定义验证，不能要求不同随机实现逐字节相等。 （完成：§43、§45）
- [x] 无法稳定分类的 f32 查询标记为“不确定”，批量返回 CPU 参考判定；记录重算比例，避免把全部任务回退误称为 GPU 提速。 （完成：`gpu/certify.rs` 认证与重算计数，§67、§77）
- [ ] 仅在上述边界通过后启用 PERF-08/09 的性能验收。 （边界已通过；性能验收需硬件 GPU，保持开放）
...
- [x] 以 `P = padded_nx*padded_ny*padded_nz`、有效 offsets 数 `K` 和原始体素数 `M` 建模：FFT 约 `O(P log P)`，直接枚举约 `O(KM)`。选择时同时考虑峰值字节与实际有效配对数，不能只比较 `M`。 （完成：`plan_exact_cpu` 成本 + 工作集模型，§68；位集内核后重拟合，§79 第 (11) 项）
- [x] 缓存维度对应的 FFT plan；使用 `process_with_scratch` 和每任务块工作区，减少反复分配。转置缓冲在重复 SA 求值间复用。（有界缓存实现与限制见 §22；完整预算/整体验收仍未完成。）
- [x] 对轴长度测试保持 `>=2N-1` 的 FFT 友好 padding；比较更快变换与更大内存之间的取舍，正确处理负位移索引。 （完成：2/3/5 平滑填充，§66）
- [x] 在同一线程池内处理填充、功率谱、转置和归一化；按块改善访问局部性，测量内存带宽后再考虑额外布局转换。 （完成：FFT 按 y 平面带聚集，§76）
- [x] 直接法可按 offset 与 voxel tile 分解；后续探索位集移位 AND + popcount，但必须正确处理跨字边界、行边界及合法配对范围。 （完成：`OccupancyBits` 移位 AND + popcount，§79 第 (11) 项）
- [ ] 若考虑实数 FFT 或 GPU FFT，作为独立实验项验证数值和资源成本，不假定当前依赖已经提供所需接口。 （未开展：可选独立实验，需新增依赖，列为负责人可选项）
...
- [x] 共享设备后让 occupancy buffer 直接供 shell kernel 使用；占据总量在 GPU 做整数归约，只回读 VF 所需计数和最终曲线。 （完成：§48、§49）
- [x] 以 `(offset, voxel_tile)` 为任务单元，workgroup 内归约 tile 命中数，再跨 tile 合并。总工作量仍约 `O(KM)`，但不再把 `M` 的循环压在单 invocation 上。 （完成：实验路径 §45、§46，生产仍为 direct，待硬件实测后选择）
- [x] 合法配对数由三轴重叠长度相乘得到，避免对每个合法体素逐次累计 `valid`；主机检查完整网格 u32 乘积，重叠子体积和 hit 计数随之有界。独立原始计数 oracle 与 release 对照见 §43。
- [x] 先得到每个 offset 的完整整数计数，再按 PERF-05 的等权规则做壳平均。禁止在 tile 层先平均比例，因为各 tile 的有效样本数不同。 （完成：§45）
- [x] offsets 按预算分批，半径归约跨批累积；没有合法配对的 offset 直接过滤，保留无支持半径的既有插值约定。 （完成：§47、§52）
- [ ] 大半径/大量 offsets 时与 CPU FFT 比较；GPU direct 并不天然优于 CPU FFT。GPU FFT/位集仅在成本模型支持时进入后续实验。 （需硬件 GPU 才有意义，保持开放）
...
- [x] 周期边界优先保存 `(particle_id, image_shift)`，避免复制整份顶点。只查询可能影响候选的 image；对角跨界、域宽附近的大粒子和重复 image 必须有参考对照。 （完成：legacy pack 模式 3 惰性 image，§66）
...
- [x] 记录桶占用分布、每次重复引用数和候选/窄相位命中比例，再调整 cell size。 （完成：`[GridStats]`，单元大小实测无收益不调整，§79 第 (8) 项）
- [x] 宽粒径比确实导致退化时，比较分级网格、粗细两层或动态 AABB 树。跨层查询必须覆盖完整 bbox 和 gap；不能只查粒子中心所在格。 （结案：候选过滤开销可忽略，无退化证据，§79 第 (8) 项）
...
- [x] 先按分量并行生成 `Metrics`：bbox、volume、surface area、aspect/sharpness 所需值。保持每分量内部原求和顺序，结果按原分量顺序收集。 （完成：§27）
- [x] 过滤阶段只读取缓存指标，维持过滤顺序及统计口径；lognormal 重平衡的 RNG 和删除顺序保持串行。 （完成：§27）
- [x] profile 显示分量发现占主导时，将 vertex-to-face 邻接改为 count → prefix sum → contiguous indices 的 CSR，减少小分配；保持“共享顶点连接”的现有定义。 （完成：CSR 邻接，§66 PERF-15）
- [x] 并查集/并行连通分量作为后续选项。即使集合相同，源 shell 的首面顺序、顶点 remap 和几何 digest 也可能改变；先为受该共享函数影响的 placement 建立身份回归。 （结案：CSR 后分量发现已非主导，未开展）
- [x] 输出多文件采用少量并发 writer，文件名/序号预先按分量顺序分配。 （完成：§27；小文件串行写出 §72）
...
- [ ] PNG 编码/写盘与下一视图渲染在小队列中重叠，保留顺序及错误传播。分辨率较小时计入初始化和读回成本来选择 auto。 （CPU/GPU 重叠已实现，§56、§66；auto 成本选择待负责人决定 (c)，重叠去留待决定 (b)）
...
- [x] 为 PERF-01/02 增加执行观察点，测试实际后端、方法与 worker 数；不要只测试配置反序列化和打印文本。 （完成：CLI 与库测试读回实际后端/方法/worker 数）
- [x] 给 PERF-04/05/14 的边界建立最小反例。小规模在 CPU/llvmpipe 上可运行，大规模资源限制优先用 planner 测试，避免为测试故意分配不可控资源。 （完成：§12、§78）
- [x] GPU 测试区分“未发现设备而跳过”“软件 adapter 执行”“硬件 adapter 执行”。专门的硬件验证任务无硬件时应明确未完成，不能绿色代替。 （完成：§79 第 (3) 项）
- [ ] 加入真实硬件 release 基准，尽量覆盖一类集成显卡和一类独立显卡；当前机器不具备这些条件，相关验收保留待完成。 （需硬件，保持开放；`hardware_gpu_acceptance` 已就绪）
- [x] 每个实施变更同步 `AI-FUNC-SUMMARY`、`AGENTS.md`、对应英文文档和中文镜像。当前新增计划文件不代表函数契约已变更。 （持续执行）
...
## 69. PERF-14 crop 轴定号修正（2026-09-25，本地，解决 §67 第 3 项）

负责人要求：若 180° 旋转合理则接受，否则保持原朝向。**判断：不合理。** 真实 CT 的旧帧（serial PCA）是绕扫描 z 轴 +50° 的纯旋转——z 保持 +z、样本保持直立，是与扫描轴最接近的帧；§66 的“每列绝对值最大分量为正”规则看到第 1 列 (−0.766, 0.643, 0) 的最大分量为负而将其翻转，行列式随之为负，再把第 2 列翻成 −z，结果是绕第一主轴 180°，样本倒置、切片方向反转。这不是更规范的朝向，只是另一种任意选择。
...
## 75. 复用局部 BVH（负责人已批准）——实测无收益，已撤回；批次上限复测（2026-09-25，本地）

**做法（已实现并测量，未保留）。** §71 的插桩显示 placement 每次尝试构建候选 TriMesh 占评估时间约 29%。parry 0.19 的 `TriMesh::transform_vertices` 会整体重建 QBVH（`rebuild_qbvh` → `clear_and_rebuild`），克隆后变换不能省下构建，所以唯一可行的方式是位姿查询：每个颗粒按 (壳层, 本颗粒尺度) 惰性构建一次局部 TriMesh（按壳层的 `OnceLock`，并行评估共享），候选以 `Isometry`（旋转 + 中心）参与相交、嵌套与间隙判定；新增 `PosedShape` 与 `posed_*` 谓词，未设位姿时逐位走原路径，并有“局部+位姿 与 世界坐标网格判定一致”的测试（150 个随机位姿，相交/双向嵌套/远离边界的间隙全部一致）。

**结果：** VF 0.30 交替 6 次中位，对照 `78f13c1`：1 worker 0.98×，4 worker 1.11×（更慢），8 worker 1.09×（更慢）；placement 输出、27,898 次尝试与拒绝计数与对照完全相同（位姿与世界坐标的舍入差异在本数据上没有改变任何决定）。原因：每次位姿查询都要把另一侧的节点包围盒与三角形变换到本侧坐标系，这部分开销与省下的构建相当；而稠密颗粒在约 126 次尝试中会抽到约 30 个不同壳层，惰性缓存只把构建减少约 4 倍。按“无收益的改动不保留”撤回全部代码（collision.rs、placement*.rs 与测试），结论记录于此。若将来要再试，需要同时减少位姿查询的变换成本（例如只变换查询点而非整棵树），否则不值得。

**批次上限复测：** 评估变便宜后批末屏障占比上升，试了 `SPECULATIVE_BATCH_PER_WORKER` = 16、32 对 8：同一配置三轮之间 1.4～2.8 s 波动，没有一致的赢家，保持 8。本机（WSL2）的单次运行波动已大于剩余可争取的收益，不设屏障的有序流水暂不实施。

## 76. PERF-07：并行 FFT 的内存翻倍（2026-09-25，本地，负责人要求优化）

§72 的大输入矩阵中 measure 峰值 RSS 从 1 worker 的 131 MiB 跳到 2 worker 以上的 255～267 MiB。按方法拆分（pitch 0.5、r_max 20，网格 100³、FFT 填充 200³）：monte_carlo 15～30 MiB，exact 131/255/269 MiB（1/2/8 worker）——全部来自 exact 的 FFT。原因：`FftWorkspace::transform` 在多任务时为 x 轴（跨步 ny·nz）分配了与网格等大的转置缓冲（200³ × 16 B = 128 MiB），整网格收集、变换、写回；单任务路径逐行处理，不需要它。
...
**第 (3) 项：GPU 测试与日志区分“跳过/软件/硬件”（已实施）。** `gpu::classify_adapter`：设备类型为 CPU，或名称为已知软件光栅化器（llvmpipe、lavapipe、SwiftShader、softpipe、Microsoft Basic Render）判为软件，否则为硬件；`GpuContext::adapter_class`/`describe`。共享设备创建时打印一次 `[Info] GPU adapter: name=… backend=… type=… class=…`，软件适配器附注“结果精确，计时不代表 GPU 硬件”。新测试文件 `tests/gpu_adapter_class_tests.rs`：分类规则单元测试；`report_the_adapter_the_gpu_tests_run_on` 在每次 GPU 测试中打印 `[GPU-TEST] adapter …`（无适配器时打印“跳过”）；`hardware_gpu_acceptance` 默认忽略，用 `--ignored` 在软件适配器上运行时**失败**并写明“hardware GPU acceptance not run”，在硬件上比较 GPU exact 与 CPU exact 的 S2（≤1e-12）并打印两者耗时。本机实测：软件适配器下该测试按设计失败。

**第 (4) 项：GPU 传输与等待计数（主机侧已实施；内核计时属硬件项）。** `runtime` 增加进程级计数：所有上传经 `CountedWrite::write_counted`（`src/gpu` 中 `write_buffer` 全部替换），所有映射回读（`read_u32_prefix` 与两处渲染回读）记录字节、次数与阻塞等待时间，共享设备初始化计时。`main` 在创建过 GPU 设备的运行结束时打印 `[Timing] gpu init_seconds=… upload_bytes=… upload_calls=… readback_bytes=… readback_calls=… readback_wait_seconds=…`。等待时间是执行加传输，明确不称为内核时间；真正的内核计时需要 `TIMESTAMP_QUERY` 与逐 pass 的时间戳写入，只在硬件上有意义，列入硬件项。测试 `transfer_counters_track_uploads_and_readbacks`：一次 MC 评估使上传（≥ 面数 × 100 字节）、回读与等待计数增加，GPU measure 的 CLI 输出含非零上传的 `[Timing] gpu` 行。

**第 (6) 项：设备丢失——审计时已存在，无需新增。** `tests/gpu_device_cache_tests.rs::shared_device_and_pipeline_cache_contract` 已用 `device.destroy()` 触发丢失，断言 `is_lost()`、缓存逐出、新建设备（计数 2）且所有管线在新设备上可重建；调用方在错误时按既有 `cpu_fallback` 策略回退（§14）。
...
**第 (8) 项与 PERF-13：placement 网格统计与单元大小（已实施/已测定）。** 审计时漏看：标签生成早已在 `[Info] Voxel label queries` 行打印 `bbox_tests=`。本批补上 placement 的 `[GridStats]`：占用分布行与 `grid_queries`/`grid_candidates` 计数（并行评估中以 relaxed 原子量累计，含投机评估，仅诊断）。VF 0.30（`data/input/placement_config.yaml` 改目标）：27 个桶、非空桶平均 18.1 个、最大 53，每次查询约 130 个候选。把单元缩到 1/2、1/4（125、729 桶）使候选数 4.15M → 3.46M → 1.47M（8 线程），而耗时 1.97/1.84/1.90 s（8 线程）、3.24/3.56/3.32 s（1 线程）无差别——廉价的球/盒测试不是瓶颈，瓶颈在精确检查（§71）。因此 PERF-13 的“据统计调整单元大小”以“不调整”结案；“宽粒径比的分级网格”同样没有依据（候选过滤的开销可忽略），不实施。另注意：改变单元大小会改变邻居顺序，从而可能改变被拒绝尝试所记录的拒绝原因（不改变接受与否），这也是不应无收益地改动它的理由。

**第 (9) 项：crop 全数值类型端到端与大体积（已实施），并由此发现并修复两个 GPU 分块问题。** 新测试 `crop_handles_every_integer_type_end_to_end`：RAW 输入覆盖 U8/I8/U16/I16/U32/I32、小端与大端（16/32 位），最近邻与三线性；断言输出保持输入类型、值域在 [min(0, fg), max(0, fg)]、最近邻在所有类型与字节序下得到相同前景掩模。大体积：合成 512×512×256 的 16 位 RAW（128 MiB，倾斜长条，生成脚本与日志在 `data/output/performance/20260925-local-matrix/large-crop/`）。CPU：1 worker 0.69 s、8 worker 0.29 s，输出一致；峰值 RSS 约 600 MB——`Volume3D` 以 i64 存储每个体素（16 位数据占 8 字节），这是大体积内存的主因，列为剩余项。GPU（64 MiB 预算，软件适配器）暴露两个问题并已修复：
- `transform_tile` 把**整个源体**对照设备缓冲上限检查（512·512·256·4 B = 256 MiB），而实际只上传 halo 子块——恰好拒绝了分块要处理的大体积。改为只要求各维度为正且在 i32 内（着色器按子块局部索引）。
- 修复后新增的传输计数显示上传 72.2 GB：规划器按“z 板→行组→x 段”取第一个放得下的形状，得到 2,178 个 461×7×1 的薄片，旋转后每片的源包围盒几乎覆盖整个长条。现在 `plan_crop_gpu_tiles` 让原分级搜索（`plan_crop_gpu_levels`）与近立方块（边长 2 的幂，≥4，≤65,536 块）按总上传源体素（`CropTilePlan::upload_voxels`）竞争，取最少者。结果：4 个 128×121×121 块，上传 71 MB（约 1/1000），变换 90.6 s → 8.5 s（软件适配器），halo 重试 0。GPU 与 CPU 最近邻输出 6,749,501 体素中 75 个不同（1.1e-5），为 f32/f64 的舍入平局，与既有斜向测试的允许情形相同。
...
**第 (11) 项：CPU direct 位集内核（已实施，成为默认）。** `OccupancyBits` 把占据场沿 z 打包为每 (x, y) 一行补零的 u64（多一个零字，保证移位读取不越界且 z ≥ nz 的位为零），`direct_pair_counts_bits` 对每个偏移逐行做移位 AND + popcount；dz < 0 时交换两端行，使每行都读取非负移位。bool 扫描 `direct_pair_counts` 保留为仅测试用的参照路径（`packed_pair_counts_equal_the_bool_scan`：z 长度 1/63/64/65/130、随机/全满/全空占据、全部域内偏移逐项相同）。单线程基准 `packed_direct_kernel_benchmark`：64³ r6 7.3–7.9×、96³ r4 6.7–6.9×、128×128×32 r4 4.1–4.2×（短行的逐行开销）、48×48×200 r5 9.8–10.2×。成本模型重拟合（24 个用例，新增 48³/64³ r16 两个 FFT 区间用例）：`NS_PER_DIRECT_PAIR` 1.35→0.115、`DIRECT_PARALLEL_EFFICIENCY` 0.6→0.3、`FFT_PARALLEL_EFFICIENCY` 0.18→0.16，最坏选错损失 3.4%；旧常数会让 64³ r6 选 FFT（0.119 s 对直接法 0.019 s）。两种内核输出逐位相同，因此方法选择变化不改变结果。工作集估计计入打包副本。

**第 (15) 项：文档行号与 clippy（已完成）。** 函数索引与各参考文档的源码行号按定义位置自动刷新（首次 1,839/3,021 行过期，脚本只在唯一匹配时改写，9 行无法唯一定位保持原样）。`src/meshgen` 与 `tests/meshgen_*` 之外的 clippy 警告在默认与 GPU 特性下均为 0：自动修复（`is_multiple_of`、`if` 合并、`expect_err`、`as_deref`、`Range::contains`）；GPU 专用字段/方法以 `cfg_attr` 标注；仅测试使用的 `compute_s2_shell_resident` 改为 `#[cfg(test)]`；`!(x <= y)` 改为显式 `x > y || x.is_nan()`（保持 NaN 语义）；公共 API 的长参数列表与 `Vec3::add/sub` 以局部 `allow` 标注。meshgen 测试中的 7 条警告按约定不改动。

**第 (10) 项与 crop 内存：`Volume3D` 带类型存储（已实施）。** `Volume3D<T = i64>` 以 `Voxel` trait（u8/i8/u16/i16/u32/i32/i64，`to_i64` 与带检查的 `from_i64`）为样本类型；默认 `i64` 让 placement 标签与既有测试不变。新增带类型加载器 `load_raw_folder_typed`、`load_tiff_or_folder_typed_with_range` 返回 `AnyVolume`：RAW 直接解码为自身类型，TIFF 直接移入解码器的带类型缓冲（不再逐值扩宽复制），页/文件间类型不一致仍报错；原扩宽加载器改为 `typed(...).map(into_i64)`。写出器对 `T` 泛型，仍按 `numeric_type` 范围检查。crop 加载后对 `AnyVolume` 匹配一次，`crop_typed::<T>` 中背景、PCA、重采样、GPU 上传、修边、写出全部对 `T` 单态化，输出保持输入类型；重采样值经 `narrow_sample` 写回（背景是输入值、最近邻返回输入值、三线性为凸组合取整，因此必定可容纳）。512×512×256 u16 RAW：CPU 8 worker 峰值 RSS 616→171 MB、总时间 0.294→0.138 s，1 worker 156 MB、0.69→0.44 s；GPU（软件适配器）821→377 MB；三者输出与旧 i64 路径逐字节相同。输出现为输入类型（约 13 MB），分块写出已无必要，第 (10) 项的“输出分块写出”以此结案。

**岛间 × 岛内 S2 组合测量（§5 PERF-02，已完成）。** 8 worker 共享池、种子 7、每岛 800 次迭代、网格 MC（8,000 样本），同一 chain 输入（剪枝后 4 个颗粒），三次重复的 anneal 吞吐（岛·迭代/秒）：1 岛 968–1,068（仅岛内 S2 并行），2 岛 1,482–1,891，4 岛 2,473–3,234，8 岛 2,357–4,063。4 岛前近似线性增长，8 岛相对 4 岛增益不稳定（样本离散度大于差值），未见池空闲或过度订阅的退化：`run_island_batches` 把在途岛数限制为 worker 数，岛内 S2 使用同一池，不需要调整调度。配置与日志：`data/output/performance/20260925-local-matrix/islands/`。该输入规模小，结论只说明调度无病态，不代表大输入的最佳岛数。
