// AI-FUNC-SUMMARY: Test-only per-sample twin of the certified MC shader (identical sampling and certified predicate, no workgroup reduction), used to verify the reduction exactly.
...
// AI-FUNC-SUMMARY: Test-only per-sample entry point: one invocation per logical sample of a single radius batch writes its own certain hit/valid flags, so host sums verify the production workgroup reduction.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let counts = sample_counts(idx / params.samples_per_radius, params.radius_base * params.samples_per_radius + idx);
    out_hits[idx] = counts.x;
    out_valids[idx] = counts.y;
}
