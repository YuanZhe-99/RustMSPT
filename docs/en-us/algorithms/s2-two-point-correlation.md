...
**Per-triangle constants (§77).** The ray direction is fixed per dispatch, so everything in the test that does not involve the query point - the edges `e1`, `e2`, the magnitude bounds `m` and `es`, `h = d x e2`, `det = e1 . h` and `eps_det` - is computed once per triangle on the host (`certify::triangle_constants`) and read from a 16-float-per-triangle buffer. The bounds were derived for correctly rounded f32 arithmetic in any evaluation order, so they hold for the host computation just as for the shader's; `precomputed_terms_respect_their_error_bounds` checks the f32 determinant against its f64 value on every adversarial fixture. Certified answers remain exactly the CPU's, and uncertain queries are still recomputed there.
