...
4. **Main loop** — `while (current_volume / box_volume) < target && attempts < max_attempts`:
   ...
   - **Placement attempt loop** (inner `loop`), for each proposal in rotation (`proposal_index % proposals.len()`, or, when a target distribution is active, continuing indefinitely by cycling through proposals until a bin choice is exhausted or `max_attempts` is hit):
     ...
     - Builds a `collision_set` = all placed meshes, extended with `generate_periodic_ghosts` of each placed mesh when `packing.mode == 3` (periodic boundary mode). Runs a parallel (`thread_pool.install`) bounding-box-pruned exact collision check (`mesh_collision_exact`); when `min_neighbor_distance` is set, bbox-distance pruning uses that threshold instead of plain overlap.
