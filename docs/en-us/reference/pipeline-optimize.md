...
- **Algorithm — per-iteration loop** (up to `params.max_iterations`, one particle perturbed per iteration):
  ...
  5. **Collision check (own particle):** queries `SpatialGrid` for neighbors within `min_neighbor` margin of the candidate's bbox, falling back to checking every other particle for any missing bbox. For each neighbor, first uses a cheap bbox-distance short-circuit, then exact `mesh_collision_exact_prepared` (parry3d) for true overlap, and — when `min_neighbor > 0.0` — exact `mesh_distance_exact_prepared` to enforce the minimum gap. Any overlap or under-gap sets `blocked = true` and aborts the candidate on rejection cooldown (same as step 4).
