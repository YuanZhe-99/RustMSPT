...
**Grid statistics (PLAN.Performance.md@1349c46 §79 item 8).** After placing, the run prints `[GridStats] placement buckets=.. non_empty=.. max_occupancy=.. mean_occupancy_non_empty=.. memberships=.. items=..` and `[GridStats] placement queries grid_queries=.. grid_candidates=..` (relaxed atomic counters from the parallel evaluations; they include speculative evaluations and never influence a decision). Label generation already prints its per-voxel bbox tests on the `[Info] Voxel label queries` line. At VF 0.30 the grid has 27 buckets holding 18 particles on average (~130 candidates per query); cells 2x and 4x finer cut candidates up to 2.8x with no change in run time, so the cell size (largest particle extent plus the gap) stays.
