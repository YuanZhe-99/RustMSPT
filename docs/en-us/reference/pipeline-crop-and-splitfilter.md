...
- **Returns:** `Ok((Volume3D, tiles))` with the same shape/semantics as the CPU path and the number of dispatched tiles, or `Err(String)` for initialization, planning (`"... no single minimal tile fits (needs at least N bytes)"`), dispatch, readback or halo-guard failures. A tripped halo guard (`volume_transform::HALO_GUARD_ERROR`: the shader sampled a voxel outside the uploaded block) is not an error by itself: the tile reruns with its block grown by 1, 2, 4... voxels per side, clamped to the source, as long as the grown block still fits the budget and the device buffer limit, and only a guard that trips on the whole source or on a block that no longer fits is returned. The count appears as `halo_retries=` in the `[Info] GPU volume transform` line; the planner's margin has never tripped it in testing. `rotate_and_crop_gpu_with(..., first_block_shrink)` exists so `halo_guard_retries_with_a_grown_block_and_keeps_the_result` can shrink every tile's first block and prove the retried output equals the untiled dispatch (PLAN.Performance.md@1349c46 §78).
...
- **Tiling:** Tiling (§79): `plan_crop_gpu_tiles` returns one tile when the output fits; otherwise the original level search (`plan_crop_gpu_levels`: z slabs, row groups, x runs) competes with roughly cubic tiles (power-of-two sides down to 4, at most 65,536 tiles) on total uploaded source voxels (`CropTilePlan::upload_voxels`, ties to fewer tiles). On a 512x512x256 16-bit volume with a rotated bar at a 64 MiB budget this took 2,178 tiles of 461x7x1 uploading 72.2 GB to 4 tiles of 128x121x121 uploading 71 MB, and the transform from 90.6 s to 8.5 s on a software adapter. `transform_tile` also no longer checks the whole source against the buffer limit - only the halo block is uploaded - which had refused exactly the volumes tiling exists for; source dimensions only need to fit i32.
