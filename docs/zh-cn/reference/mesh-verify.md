...
| `verify_with_options` | `src/meshgen/verify.rs:441` | 带文档之外选项的 `verify`；面阶段 s00-s03 会跳过仅适用于体网格的 [V7]/[V8]。 |
...
目录始终报告全部**十二**节。凡是所需数据当前文档并不携带的检查，都会以
...
| [V12] 溯源与统计 | **完整** | 计数、元数据回显、`Counts` 与网格实际值一致性、`SchemaVersion` 校验、`StageIndex` 范围与文件名交叉校验（T-C6） |
