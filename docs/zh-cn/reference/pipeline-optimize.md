...
- **算法 —— 逐迭代循环**（最多 `params.max_iterations` 次，每次迭代扰动一个颗粒）：
  ...
  5. **碰撞检查（自身颗粒）：** 在 `SpatialGrid` 中查询候选颗粒包围盒 `min_neighbor` 边距内的邻居，若有包围盒缺失则回退为逐一检查所有其他颗粒。对每个邻居，先使用开销较低的包围盒距离快速判定，再使用精确的 `mesh_collision_exact_prepared`（parry3d）判定真实重叠，并在 `min_neighbor > 0.0` 时使用精确的 `mesh_distance_exact_prepared` 强制执行最小间隙。任何重叠或间隙不足都会将 `blocked` 置为 `true` 并按拒绝冷却流程中止该候选（与第 4 步相同）。
