...
| `mc_method` | `"exact"`（体素网格光线投射）、`"monte_carlo"`（随机采样），或 `"both"`（两者都运行并报告其 L2 距离）。若体素网格超过 1,500,000 个体素，`"exact"` 会自动降级为蒙特卡洛方法。 |
...
体素，远低于精确方法的 1,500,000 体素降级阈值，也远低于 250,000 体素的 GPU 阈值，因此在
