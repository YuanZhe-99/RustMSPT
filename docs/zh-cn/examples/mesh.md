...
每个快照都写成**一对**文件（P-2.1）。`mesh_s08_cut.vtu` 就是网格本身：仅含四面体，并在已贴合几何的单元上携带真实的 `region_key`。在 ParaView 中直接打开时，它的 Feature Edges 只有外层区域盒——这正是它拥有普通文件名的理由。紧邻其旁的 `mesh_s08_cut_contract.vtu` 是混合单元契约文档：同样的四面体，外加带标签的界面三角形（`cell_kind = 1`）及其 `(内侧, 外侧)` 单元对，以及边缘折线。当你需要这些标签时打开它；当你想让 `mesh-verify` 跑完整个检查目录时也交给它，因为 `[V5]`–`[V9]` 要读这些标签。两个文件共享同一个点数组。
