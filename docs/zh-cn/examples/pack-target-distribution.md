...
> **本页的每一个数字都只是某一次运行的结果。** `packing:` 引擎不接受种子，因此颗粒数、各区间实际值与尝试
> 次数在同一份配置的不同运行之间都会变化；只有体积分数被停止规则钉住，且只钉住下界。本页要讲的是这张对照表
> 的形状，而非具体位数。参见 [pack.md](pack.md) 中的同一条说明。自 0.2.1 起，本循环同样会拒绝整体位于已放置
> 颗粒内部的候选。
