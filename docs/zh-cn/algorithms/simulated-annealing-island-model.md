...
**增量体素占据（PERF-10 第 5 项）。** `voxel_mc` 与 `voxel_exact` 下每个岛维护一个 `VoxelCoverage`：每个体素记录包含其中心的颗粒连通分量数量，占据 = 计数 > 0。可行候选只重新查询被移动颗粒的分量（与 `build_bbox_occupancy` 相同的 `PreparedMeshQuery` 包含测试、体素中心和截断范围，经 `part_voxel_ranges` 共用），先减去旧列表再加上新列表，因此不会清除仍被其他颗粒覆盖的体素；拒绝时恢复保存的列表，不做任何查询；迁移及每第 64 次更新全量重建。S2 由 `OptimizeS2::evaluate_voxel_grid` 在该网格上计算，它与 `calculate_s2` 对合并网格体素化得到的网格完全相同（`incremental_coverage_matches_full_voxelization` 逐步检查接受/拒绝、重叠、嵌套、跨界、刷新、移除与迁移序列，含 exact S2）。release 基准：0.26～2.1 M 体素下单步占据更新比全量体素化便宜 40～340 倍；60^3 盒、pitch 0.5 的 300 次迭代 voxel_mc 运行退火阶段由 115 s 降至 3.9 s。代价是每体素一个 `u32` 计数及每颗粒的覆盖体素列表（该运行峰值 RSS 36 -> 55 MB）。`INCREMENTAL_VOXEL_OCCUPANCY` 可关闭。

**GPU 局部上传（PERF-10）。** GPU MC 三角形上传现在将新的合并网格与驻留缓冲的主机影子副本逐位比较，只写入发生变化的面区间（移动一个粒子即只上传该粒子的面）；三角形数量变化、缓冲扩容、区间超过 64 段或变化面超过一半时回退为整体写入；`GpuS2Pipeline::upload_stats` 记录字节数。体素占据改为增量维护（见下）。
