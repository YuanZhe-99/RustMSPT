...
后端由 `acceleration` 和 `RUSTMSPT_ACCELERATION` 共同选择。auto 使用 `gpu_min_voxels`（默认 250,000 个输出体素），显式 GPU 不受该阈值限制。GPU 变换按逻辑预算（`gpu_memory_limit_mb`）和设备单缓冲上限规划输出分块：放得下时整个输出一块，否则依次使用最大的完整 xy 的 z 板、单切片内的行组、单行内的 x 段。每块把 8 个输出角点逆映射到源空间取 AABB，加插值 halo（floor..floor+1，覆盖三线性邻点和最近邻舍入）与 f32 舍入余量，裁剪到体内后只上传该子块——旋转会跨越源切片，所以绝不按输出 z 范围切源。着色器按绝对输出索引重算每个体素并按完整源尺寸判越界，因此两种插值的分块输出都与单次 dispatch 逐字节一致；守卫字把任何越出已上传子块的读取变成错误。交给策略的工作集估计是计划峰值（2×最大子块 + 最大输出块 + staging + 守卫 + 160 字节参数）；只有单体素块也放不下时，预算才导致回退 CPU（auto）或报错（gpu）。数值不支持、设备不可用、预算不足或运行失败时，只有 `cpu_fallback` 允许才回退。最近邻要求值可无损表示为 i32；三线性还要求输入与背景可精确表示为 f32。宽 U32 标签留在 CPU，禁止回退时明确报错。半整数采用与 CPU 一致的远离零舍入，但不保证任意 f32 变换与 f64 CPU 逐位一致。二维 dispatch 覆盖超出单行上限的输出，并遵循 `RUSTMSPT_GPU_DEVICE`。CPU 在深度不足以利用线程池时采用约 4096 体素的行分块，否则保持切片任务，在配置线程池中执行。
...
`pca_frame(cov)` 按特征值降序排序，并把相邻差值不超过最大模 `PCA_DEGENERATE_REL_TOL = 1e-3` 倍的特征值归为一组。三重组直接取扫描轴。二重组按 x、y、z 顺序把扫描轴投影到该特征空间并 Gram-Schmidt 正交化，只接受残差范数不小于 0.5 的轴（总能找到两个），因此基不再依赖求解器噪声。非退化的第 k 列定号为其沿扫描轴 k 的分量为正，即选取最接近扫描轴的坐标系；仅当该分量小于 `PCA_SIGN_DIAGONAL_TOL = 1e-6` 时才改由绝对值最大分量决定（恰好相等时取最低轴）。行列式为负时翻转对角分量绝对值最小的非退化列（放弃的旋转最少）；全部为退化列时翻转第 2 列。测试：立方体和球（含与不含额外一个体素）得到单位坐标系；沿 z、x 的圆柱（含与不含额外体素）分别得到 `[z, x, y]` 与单位阵；倾斜的二重协方差加 1e-9 噪声后坐标系不变。

非退化列的定号约定是必要的：普通斜向样本的三遍与单遍协方差仅末位不同，`SymmetricEigen` 的特征向量符号就发生了翻转。第一版把每列绝对值最大分量定为正；在仓库真实 CT RAW 数据上（主轴在扫描平面内与 x 轴成 50 度），这翻转了第 1 列，右手性修正又把第 2 列变为 -z，裁剪结果被旋转 180 度且切片方向反转。按对角分量定号后恢复为绕 z 轴 50 度的纯旋转，裁剪输出与 2026-09-25 之前的基线逐字节一致（PLAN.Performance.md@1349c46 §68）。测试：`frame_keeps_the_scan_orientation_of_a_sample_rotated_about_z`。

release 基准（`pca_online_benchmark`，5 个样本中位数，每样本 10 次，三遍 -> 单遍秒数）：205,349 体素 1/2/4 workers 0.005596->0.003662、0.005614->0.003663、0.004742->0.003084；1,642,792 体素 0.043084->0.027811、0.025081->0.018758、0.034447->0.022170；13,142,336 体素 0.580528->0.402514、0.609000->0.408049、0.214946->0.146319。所测每种情况单遍形式均更快。
