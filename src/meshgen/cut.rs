...
pub struct CutStats {
    ...
    /// Fanned pieces whose material surface was recovered through their interior spokes.
    pub n_spoke_cut_pieces: usize,
    /// Tets those pieces emitted, cut and uncut together.
    pub n_spoke_cut_tets: usize,
    /// Interface triangles the spoke cut declared - the staircase turned into a real surface.
    pub n_spoke_cut_faces: usize,
...
pub fn cut_lattice(
    ...
    // Spoke cutting recovers a component's surface inside a fanned piece. `RUSTMSPT_NO_SPOKE_CUT`
    // turns it off, which is how it is A/B'd against the fan it replaces.
    let cutting_spokes = std::env::var_os("RUSTMSPT_NO_SPOKE_CUT").is_none();
    let surface_index: BTreeMap<i32, SurfaceIndex> = if cutting_spokes {
        all_components
            .iter()
            .filter_map(|component| {
                let slot = classifier.slot_of(*component)?;
                let triangles = classifier.triangles(slot);
                (!triangles.is_empty()).then(|| (*component, SurfaceIndex::build(triangles)))
            })
            .collect()
    } else {
        BTreeMap::new()
    };
    let mut spoke_scratch: Vec<u32> = Vec::new();
    let mut spoke_reject = SpokeReject::default();
    ...
    for (index, cell) in per_cell.into_iter().enumerate() {
        if let Some(reason) = cell.escalation {
            ...
            *mesh.stats.n_escalated.entry(reason).or_insert(0) += 1;
            ...
            if probing_spokes || cutting_spokes {
                for component in &all_components {
            ...
            for (slot, (slab, pairs)) in slabs.iter().enumerate() {
                ...
                // Recover the material surface through the fan's interior spokes, so the
                // boundary the fan would otherwise leave as an untagged staircase becomes a
                // declared interface with the ownership on each side taken from §6's table
                // instead of from a centroid sample. Only where the piece straddles: the first
                // component that both straddles it and cuts cleanly wins, and any refusal keeps
                // the plain fan below.
                let spoke = cutting_spokes
                    .then(|| {
                        for component in &all_components {
                            let Some(surface) = surface_index.get(component) else {
                                continue;
                            };
                            let empty = BTreeSet::new();
                            let known = on_surface.get(component).unwrap_or(&empty);
                            let cut = spoke_cut_fan(
                                &fanned.tets,
                                slab,
                                centroid_id,
                                centroid,
                    ...
                    .flatten();
                if let Some(cut) = spoke {
                        mesh.stats.n_spoke_cut_pieces += 1;
                        mesh.stats.n_spoke_cut_tets += cut.tets.len();
                        for triangle in &cut.interface {
                            mesh.stats.n_spoke_cut_faces += 1;
                            pending_interfaces.push((mesh.tets.len(), *triangle, cut.component));
                        }
                        for piece in cut.tets.iter() {
                            let Some(oriented) = orient_positively(*piece, &mesh.nodes) else {
                                mesh.stats.n_degenerate_fan_pieces += 1;
                                continue;
                            };
                            mesh.tets.push(oriented);
                            // Ownership stays with §7.5's interior sample, deliberately. The
                            // table's answer for the component just cut is *exact about a plane*
                            // - the three cut nodes - while the surface between them is curved,
                            // so asserting it disagrees with a centroid sample by exactly the
                            // discretisation error. Measured on A-6a: asserting it takes
                            // `misattributed_cells` 0 -> 690 and the volume error 0.499 -> 1.255 %
                            // while undeclared faces go 1,050 -> 501. That trade is the wrong way
                            // round - misattribution is a check with a defined answer, and the
                            // chamfer it would buy back is gate G6-0's accepted cost. So this cut
                            // recovers the *geometry* and leaves the *labels* where they were.
    ...
    if cutting_spokes && std::env::var_os("RUSTMSPT_SPOKE_DIAG").is_some() {
        println!(
            "[SPOKE-CUT] pieces={} tets={} faces={}  rejects: base_straddle={} no_crossing={} \
             no_table_row={} dry_run={} no_side={} degenerate_child={} inert={}",
            mesh.stats.n_spoke_cut_pieces,
            mesh.stats.n_spoke_cut_tets,
            mesh.stats.n_spoke_cut_faces,
            spoke_reject.base_straddle,
            spoke_reject.no_crossing,
            spoke_reject.no_table_row,
            spoke_reject.dry_run,
            spoke_reject.no_side,
            spoke_reject.degenerate_child,
            spoke_reject.inert,
        );
    }
    if probing_spokes {
...
/// How small a child of a spoke cut may be, as a fraction of the fan tet it came from. Below
/// this a child is a numerical artefact rather than an element: its faces are what neighbouring
/// tets disagree about, and the volume guard cannot see it because it has no volume.
const SPOKE_MIN_CHILD_FRAC: f64 = 1.0e-9;

// AI-FUNC-SUMMARY: Signed volume of one tet; returns f64; side effects: none.
fn tet_volume(tet: [u32; 4], nodes: &[Vec3]) -> f64 {
    orient3d(
        nodes[tet[0] as usize],
        nodes[tet[1] as usize],
        nodes[tet[2] as usize],
...
/// One component's surface, with a broad phase, ready to be asked where it crosses a segment.
struct SurfaceIndex<'a> {
    triangles: &'a [[Vec3; 3]],
    grid: crate::meshgen::snap::BoxGrid,
}

impl<'a> SurfaceIndex<'a> {
    // AI-FUNC-SUMMARY: Bucket one component's arranged triangles by their AABBs; returns SurfaceIndex; side effects: none.
    fn build(triangles: &'a [[Vec3; 3]]) -> Self {
        ...
        SurfaceIndex {
            triangles,
            grid: crate::meshgen::snap::BoxGrid::build(&bounds),
        }
    }

    // AI-FUNC-SUMMARY:
    // Purpose: Where this surface crosses the segment `a -> b`, when it does so exactly once.
    // Inputs: the segment's endpoints and a scratch buffer for the broad phase.
    // Returns: the crossing point; None when the surface misses the segment, only touches a
    //   triangle's edge, or crosses more than once.
    // Side effects: Clears and refills `scratch`.
    // Notes: **More than one crossing is a refusal, not a choice.** §6's table replaces a tet cut
    //   by *one* patch; a spoke the surface enters and leaves again is not that configuration, and
    //   picking one of the crossings would claim material on the far side of the other. Declining
    //   costs the piece its cut and keeps the fan, which is the outcome that was already correct.
    //   Crossings are deduplicated by position first: adjacent arranged triangles sharing an edge
    //   both report a segment passing exactly through that edge, and that is one crossing seen
    //   twice rather than two.
    fn crossing(&self, a: Vec3, b: Vec3, scratch: &mut Vec<u32>) -> Option<Vec3> {
        let lo = Vec3::new(a.x.min(b.x), a.y.min(b.y), a.z.min(b.z));
        let hi = Vec3::new(a.x.max(b.x), a.y.max(b.y), a.z.max(b.z));
        self.grid.query_into(lo, hi, scratch);
        let mut found: Option<Vec3> = None;
        for slot in scratch.iter() {
            let Some(point) = segment_pierces_triangle(a, b, self.triangles[*slot as usize]) else {
                continue;
            };
            match found {
                None => found = Some(point),
                Some(first) => {
                    let apart = point.sub(first);
                    if apart.dot(apart) > 0.0 {
                        return None;
                    }
                }
            }
...
/// Why spoke cutting declined a piece, so a refusal is diagnosable rather than silent.
#[derive(Default, Clone, Copy, Debug)]
struct SpokeReject {
    /// A fan tet's base carried both sides - the checked precondition.
    base_straddle: usize,
    /// The surface missed a spoke, touched a triangle's edge, or crossed it more than once.
    no_crossing: usize,
    /// §6's table has no row for the configuration.
    no_table_row: usize,
    /// The cut failed the volume/orientation guard.
    dry_run: usize,
    /// A tet lying wholly in the surface, which owns volume on neither side.
    no_side: usize,
    /// The decomposition produced a child too small to carry a face reliably.
    degenerate_child: usize,
    /// Attempts that found nothing to cut - the piece does not straddle this component.
    inert: usize,
}

/// What one piece's fan became after the surface was recovered through its spokes.
struct SpokeCut {
    tets: Vec<[u32; 4]>,
    /// Per tet, whether it is on `component`'s inside. Exact, from §6's table - not a sample.
    inside: Vec<bool>,
    /// The surface inside the cell, to be declared for `component`.
    interface: Vec<[u32; 3]>,
    component: i32,
}

// AI-FUNC-SUMMARY:
// Purpose: `PLAN_mesh_generation.md` - recover a component's surface inside a fanned piece by
//   cutting its fan tets on **interior spokes only**, so the cell's boundary is untouched.
// Inputs: the piece's fan tets, the apex id and position, the component and the nodes its surface
//   is known to pass through, its surface index, the classifier, the mesh's node and key arrays,
//   the key quantum, and the cut options for the volume guard.
// Returns: the recovered fan, or None to keep the piece's plain fan unchanged.
// Side effects: Appends spoke cut-nodes to `nodes` and `keys`. On None those stay unreferenced,
//   which costs a coordinate and changes nothing.
// Notes: **Why this does not need invariant J1's agreement protocol.** A fan tet's edges are
//   either walk edges between two boundary nodes, or spokes from a boundary node to the apex. A
//   spoke is strictly interior to the cell, so no neighbour shares it and a node placed on it is
//   this cell's alone. Cutting spokes only therefore leaves every shared face exactly as the
//   neighbour built it, and J1 holds by construction rather than by agreement.
//
//   That rests on a base never straddling, which is **checked, not assumed**: the probe measured
//   it true for 98 % of straddling fan tets and false for 271 of them, concentrated in A-3 and
//   A-8, because `split_soup_by_surface`'s `mixed sides` relaxation admits faces that straddle
//   without a cut node. A straddling base declines the whole piece, so the 2 % cost a chamfer
//   they were already paying instead of breaking conformity.
//
//   All-or-nothing per piece. A piece whose fan is cut in part has a material boundary that stops
//   in mid-air, which is worse than the staircase it replaces - so any refusal anywhere discards
//   the whole attempt and the caller fans the piece as before.
//
//   **Spokes are shared between fan tets of one piece.** Two fan tets over adjacent boundary
//   triangles share the face `(u, v, apex)`, whose edges are the walk edge `(u, v)` and the spokes
//   `(u, apex)` and `(v, apex)`. A cut node placed independently on each side would crack the
//   piece along that face, so they are interned per cell in `spoke_nodes`. This is the failure
//   mode that broke `[V1]`/`[V3]` in previous attempts at this code.
//
//   **A refusal must take its nodes back with it.** Unlike §7.6's abandoned cap points, an
//   abandoned spoke node lies exactly *on* a fan edge, and `[V3]`'s hanging-node test is
//   geometric - it asks whether any node lies on a face it is not a vertex of, and an
//   unreferenced one on a spoke answers yes. Leaving them cost 1,278 hanging nodes on A-6a
//   with the mesh otherwise unchanged. The wrapper truncates back to the entry mark, which is
//   sound because only this function appends here and nothing outside it can hold the ids.
#[allow(clippy::too_many_arguments)]
fn spoke_cut_fan(
    fan: &[[u32; 4]],
    slab: &[[u32; 3]],
    ...
    options: &CutOptions,
    scratch: &mut Vec<u32>,
    reject: &mut SpokeReject,
) -> Option<SpokeCut> {
    let mark = nodes.len();
    let cut = spoke_cut_fan_inner(
        fan,
        slab,
        apex_id,
        apex_point,
...
fn spoke_cut_fan_inner(
    ...
    let slot = classifier.slot_of(component)?;
    let side_of = |node: u32, point: Vec3| -> NodeSide {
        if surface_nodes.contains(&node) {
            return NodeSide::OnCut;
        }
        let mut uncertain = 0usize;
        let inside = classifier.inside(point, slot, &mut uncertain);
        if uncertain > 0 {
    ...
    let mut sides: BTreeMap<u32, NodeSide> = BTreeMap::new();
    let apex_side = side_of(apex_id, apex_point);
    sides.insert(apex_id, apex_side);

    let mut spoke_nodes: BTreeMap<u32, u32> = BTreeMap::new();
    let mut out = SpokeCut {
        tets: Vec::with_capacity(fan.len()),
        inside: Vec::with_capacity(fan.len()),
        interface: Vec::new(),
        component,
    };
    ...
    for raw in fan {
        // `fan_cell` emits its tets unoriented by contract, and the volume guard measures the
        // parent with a signed determinant - so an inverted parent reads as zero volume and
        // every cut is refused. Orienting first is safe for the slot arithmetic below because
        // `orient_positively` swaps slots 0 and 1, leaving the apex at slot 3.
        let Some(tet) = orient_positively(*raw, nodes) else {
            reject.dry_run += 1;
            return None;
        };
        let tet = &tet;
        let mut here = [NodeSide::OnCut; 4];
        for (slot_index, node) in tet.iter().enumerate() {
            here[slot_index] = match sides.get(node) {
                Some(side) => *side,
                None => {
                    let side = side_of(*node, nodes[*node as usize]);
                    sides.insert(*node, side);
                    side
                }
            };
        }
        // The apex is `fan_cell`'s last slot; the other three are the base.
        let base = &here[..3];
        let base_inside = base.contains(&NodeSide::Inside);
        let base_outside = base.contains(&NodeSide::Outside);
        if base_inside && base_outside {
            reject.base_straddle += 1;
            // The checked precondition. A base edge would need the cut, and a base edge can lie
            // in a shared face - which is J1's problem and this function's whole premise.
            return None;
        }
        let has_inside = base_inside || here[3] == NodeSide::Inside;
        let has_outside = base_outside || here[3] == NodeSide::Outside;
        if !(has_inside && has_outside) {
            // Wholly on one side: keep it, on the side it is on. `OnCut` throughout means the
            // tet lies in the surface, which owns no volume on either side - refuse rather than
            // guess, because a tet with no side has no ownership to record.
            let Some(side) = (match (base_inside, base_outside) {
                (true, false) => Some(true),
                (false, true) => Some(false),
                _ => match here[3] {
                    NodeSide::Inside => Some(true),
                    NodeSide::Outside => Some(false),
                    NodeSide::OnCut => None,
                },
            }) else {
                reject.no_side += 1;
            ...
            out.tets.push(*tet);
            out.inside.push(side);
            continue;
        }
        // Every straddling edge runs from a base slot to the apex - that is what the precondition
        // above guarantees - so every cut node lands strictly inside the cell.
        let mut failed = false;
        {
            let mut place = |base_slot: usize| -> Option<u32> {
                let node = tet[base_slot];
                if let Some(existing) = spoke_nodes.get(&node) {
                    return Some(*existing);
                }
                let point = surface.crossing(nodes[node as usize], apex_point, scratch)?;
                // A crossing that quantizes onto one of the spoke's own endpoints is not a cut:
                // the node it would add is the endpoint, §4's SNK rule then sees two vertices
                // with one key, and the two fan tets sharing that spoke can break the tie
                // differently. The result is a quad triangulated both ways - four unmatched
                // faces bounding a void of 9.9e-22, read off A-3's cell 90746, which `[V3]`
                // reports as boundary leaks and the volume guard cannot see because the void
                // has no volume to lose.
                let key = node_key(point, quantum);
                if key == keys[node as usize] || key == keys[apex_id as usize] {
                    return None;
                }
                // The cut node must be **strictly inside** the piece. Where two bodies are in
                // exact contact - A-6's limb meets the cube on a shared plane - this component's
                // surface can coincide with a cap the cell was already split along, and the
                // crossing then lands on a face of the neighbouring piece rather than inside
                // this one. That face is properly shared by its own two tets, so nothing is
                // torn; the new node simply hangs on it, which is A-6b's 10 hanging nodes.
                if slab.iter().any(|triangle| {
                    point_on_triangle(
                        point,
                        [
                ...
                nodes.push(point);
                keys.push(key);
                spoke_nodes.insert(node, id);
                Some(id)
            };
            for slot_index in 0..3 {
                let opposed = matches!(
                    (here[slot_index], here[3]),
                    (NodeSide::Inside, NodeSide::Outside) | (NodeSide::Outside, NodeSide::Inside)
                );
                if opposed && place(slot_index).is_none() {
                    failed = true;
                    break;
                }
            }
        ...
        if failed {
            reject.no_crossing += 1;
            return None;
        }
        let cut_node = |a: usize, b: usize| -> Option<u32> {
            // One of the two slots is the apex; the other names the spoke.
            let base_slot = if a == 3 {
                b
            } else if b == 3 {
                a
            } else {
            ...
            spoke_nodes.get(&tet[base_slot]).copied()
        };
        let Some(cell) = cut_tet(*tet, here, &cut_node, keys) else {
            reject.no_table_row += 1;
            return None;
        };
        let children: Vec<[u32; 4]> = cell.tets.iter().copied().collect();
        // The same guard §6's own cut answers to: every child positive, and their volumes summing
        // to the parent's. A fan tet is convex, so a cut that passes this partitions it exactly.
        let Ok((oriented, _, _)) = guarded_dry_run(*tet, &children, nodes, options) else {
            reject.dry_run += 1;
            return None;
        };
        // `guarded_dry_run` refuses a child of non-positive volume and checks the sum, so it
        // cannot see a child that is merely *negligible*: a sliver of 1e-15 of its parent sums
        // fine and still carries faces that no neighbour matches. Refuse the piece instead.
        let parent_volume: f64 = tet_volume(*tet, nodes).abs();
        if oriented
            .iter()
            .any(|child| tet_volume(*child, nodes).abs() <= SPOKE_MIN_CHILD_FRAC * parent_volume)
        {
            reject.degenerate_child += 1;
            return None;
        }
        for (child, inside) in oriented.iter().zip(cell.inside.iter()) {
            out.tets.push(*child);
            out.inside.push(*inside);
        }
        out.interface.extend(cell.interface.iter().copied());
        cut_any = true;
    }
    if !cut_any {
        reject.inert += 1;
    }
    cut_any.then_some(out)
}
...
mod tests {
    ...
    /// A unit square in the plane z = 0, as two triangles sharing the diagonal.
    fn plane_z0() -> Vec<[Vec3; 3]> {
        vec![
            [
    ...
    // The rule the spoke cut's soundness rests on: exactly one crossing is an answer, and
    // anything else is a refusal rather than a choice.
    #[test]
    fn surface_crossing_answers_only_when_the_surface_is_met_once() {
        let triangles = plane_z0();
        let index = SurfaceIndex::build(&triangles);
        let mut scratch = Vec::new();

        // Straight through the interior of one triangle.
        let hit = index.crossing(
            Vec3::new(0.5, -0.5, -1.0),
            Vec3::new(0.5, -0.5, 1.0),
            &mut scratch,
        );
        let hit = hit.expect("a segment through the interior crosses once");
        assert!((hit.z).abs() < 1e-12, "the crossing lies in the plane: {hit:?}");
        assert!((hit.x - 0.5).abs() < 1e-12 && (hit.y + 0.5).abs() < 1e-12);
        ...
        // Along the shared diagonal, where two triangles both report the same point. That is
        // one crossing seen twice, and it must not be mistaken for two.
        let on_diagonal = index.crossing(
            Vec3::new(0.0, 0.0, -1.0),
            Vec3::new(0.0, 0.0, 1.0),
            &mut scratch,
        );
        assert!(
            on_diagonal.is_none() || (on_diagonal.unwrap().z).abs() < 1e-12,
            "a segment down the shared edge is either refused or answered once"
        );
    }

    // Two parallel sheets: a segment through both is crossed twice, which §6's one-patch table
    // has no row for, so it must be refused rather than resolved to one of them.
    #[test]
    fn surface_crossing_refuses_a_segment_that_crosses_twice() {
        let mut triangles = plane_z0();
        for shifted in plane_z0() {
            triangles.push(shifted.map(|p| Vec3::new(p.x, p.y, p.z + 0.5)));
        }
        let index = SurfaceIndex::build(&triangles);
        let mut scratch = Vec::new();
        assert!(
            index
                ...
                .is_none(),
            "two distinct crossings must be refused, not resolved to one"
        );
    }
}
