...
// Each workgroup handles one offset/voxel tile, cooperatively scans overlap voxels and reduces exact integer hits.
...
fn main(@builtin(local_invocation_index) lane: u32,
    ...
    if (idx >= params.total_offsets * params.tiles_per_offset) { return; }
    let entry = offsets[idx / params.tiles_per_offset];
    ...
    let overlap = extent.x * extent.y * extent.z;
    let begin = (idx % params.tiles_per_offset) * params.tile_voxels;
    if (begin >= overlap) {
        if (lane == 0u) { out_valid[idx] = 0u; out_hits[idx] = 0u; }
        return;
    }
    let valid = min(params.tile_voxels, overlap - begin);
    var hits = 0u;
    var local = lane;
    loop {
        if (local >= valid) { break; }
        let position = begin + local;
        let z = position % extent.z;
        let row = position / extent.z;
