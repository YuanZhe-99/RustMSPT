...
impl GpuRenderPipeline {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        super::runtime::scoped(&device.clone(), || {
            let (pipeline, bind_group_layout) = shared.cached_pipeline("render", include_str!("shaders/render.wgsl"), |device| {
    ...
    pub fn render(
        ...
        {
            return Err("render resolution exceeds GPU texture limits".into());
        }
        let unpadded_bpr = width.checked_mul(4).ok_or("render row bytes overflow")?;
        let padded_bpr = unpadded_bpr
            .checked_add(255)
            .ok_or("render padded row overflow")?
            / 256
            * 256;
        let staging_bytes = padded_bpr
            .checked_mul(height)
            .ok_or("render readback bytes overflow")?;
        let vertex_count = mesh
            .faces
            .len()
            .checked_mul(3)
            .ok_or("render vertex count overflow")?;
        let vertex_bytes = vertex_count
            .checked_mul(std::mem::size_of::<RenderVertex>())
            .ok_or("render vertex bytes overflow")?;
        if vertex_count > u32::MAX as usize
            || vertex_bytes as u64 > limits.max_buffer_size
            || staging_bytes as u64 > limits.max_buffer_size
        {
            return Err("render buffers exceed GPU capacity".into());
        }
